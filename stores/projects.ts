...
export const useProjectStore = defineStore("projects", {
  ...
  actions: {
    ...
    async create(project: Omit<Project, "id">) {
      try {
        const projectId = await createObject<Omit<Project, "id">>(
          "/projects",
          project,
        );
        const action: Action = {
    ...
    async meetingScheduled(
      ...
      const meeting = await this.getCalendlyMeetingDetails(
        projectId,
        meetingUrl,
        clientUrl,
      );
    ...
    async incrementPhase(projectId: string) {
      ...
      await this.updatePhase(
        projectId,
        this.getNextProjectPhase(project.phase),
      );
    },
